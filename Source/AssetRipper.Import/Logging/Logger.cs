...
namespace AssetRipper.Import.Logging
{
	public static class Logger
	{
		...
		{
#if DEBUG
			Log(LogType.Info, LogCategory.System, $"U4X-ARipper Build Type: Debug {GetBuildType()}");
#else
			Log(LogType.Info, LogCategory.System, $"U4X-ARipper Build Type: Release {GetBuildType()}");
#endif
		}
		...
		public static void LogSystemInformation(string programName)
		{
			...
			ErrorIfBigEndian();
			Log(LogType.Info, LogCategory.System, $"U4X-ARipper Version: {typeof(Logger).Assembly.GetName().Version}");
