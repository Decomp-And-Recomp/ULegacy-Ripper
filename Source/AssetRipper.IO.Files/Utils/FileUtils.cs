...
namespace AssetRipper.IO.Files.Utils
{
	public static class FileUtils
	{
		...
		public static bool ExistsIgnoreCasing(string? path)
		{
			if (path == null || string.IsNullOrEmpty(path)) return false;
			foreach (string path2 in Directory.GetFiles(Path.GetDirectoryName(path))) {
				if (path2.Equals(path, StringComparison.OrdinalIgnoreCase)) {
					return true;
				}
			}
		...
		public static string GetUniqueName(string dirPath, string fileName, int maxNameLength)
		{
			...
			{
				if (!ExistsIgnoreCasing(Path.Combine(dirPath, validFileName)))
				{
					return validFileName;
				}
			}
			...
			{
				string proposedName = $"{name}{ext}";
				
				if (!Directory.Exists(Path.Combine(dirPath, counter.ToString())))
				{
					Directory.CreateDirectory(Path.Combine(dirPath, counter.ToString()));
				}

				if (!ExistsIgnoreCasing(Path.Combine(dirPath, counter.ToString(), proposedName)))
				{
					UniqueNamesByInitialPath[key] = counter;
					return Path.Combine(counter.ToString(), proposedName);
				}
			}
